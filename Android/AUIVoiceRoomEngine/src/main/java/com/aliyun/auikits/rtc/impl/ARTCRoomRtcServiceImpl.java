...
public class ARTCRoomRtcServiceImpl extends AliRtcEngineEventListener implements ARTCRoomRtcService {
    ...
    public static final class RTCAudioVolumeObserver extends AliRtcEngine.AliRtcAudioVolumeObserver{
        ...
        public void onAudioVolume(List<AliRtcEngine.AliRtcAudioVolume> list, int i) {
            ...
            obj.mUIHandler.post(new Runnable() {
                ...
                public void run() {
                    if(obj.mRtcObserver != null)
                        obj.mRtcObserver.onSpeakerActivated(targetUser, speaking[0], pitch[0]);
                }
            });
        }
    }
    ...
    public void initEngine() {
        ...
        if (mRTCEngine != null) {
            ...
            if(mMode == ClientMode.KTV){
                mRTCEngine.setAudioProfile(AliRtcEngine.AliRtcAudioProfile.AliRtcEngineStereoHighQualityMode, AliRtcEngine.AliRtcAudioScenario.AliRtcSceneKtvMode);
            }else{
                mRTCEngine.setAudioProfile(AliRtcEngine.AliRtcAudioProfile.AliRtcEngineStereoHighQualityMode, AliRtcEngine.AliRtcAudioScenario.AliRtcSceneMusicMode);
            }
    ...
    public int switchMicrophone(boolean open) {
        ...
        if(mRTCEngine == null)
            return -1;
        int rs = mRTCEngine.muteLocalMic(!open, AliRtcEngine.AliRtcMuteLocalAudioMode.AliRtcMuteAllAudioMode);
        if (open) {
            mRTCEngine.startAudioCapture();
        } else {
            mRTCEngine.stopAudioCapture();
