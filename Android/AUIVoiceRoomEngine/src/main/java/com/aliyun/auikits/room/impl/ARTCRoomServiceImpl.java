...
public class ARTCRoomServiceImpl implements ARTCRoomServiceInterface {
    ...
    public int dismissRoom(String roomId) {
        ...
        return IMService.getInstance().sendGroupMessage(roomId, IMParams.MSG_TYPE_DISMISS_ROOM, dismissJson.toString(), true, true);
    }
    ...
    public void leaveMic(LeaveMicInfo leaveMicInfo, ActionCallback callback) {
        ...
        //发送麦位信息同步通知
        IMService.getInstance().sendGroupMessage(leaveMicInfo.roomInfo.roomId, IMParams.MSG_TYPE_SYNC_LEAVE_MIC, jsonObj.toString(), true, true);
    ...
    public int kickOutMic(KickOutMicInfo kickInfo) {
        ...
        //发送踢人消息
        IMService.getInstance().sendMessage(kickInfo.roomInfo.roomId, kickInfo.userInfo.userId, IMParams.MSG_TYPE_KICK_OUT, jsonObject.toString(), true, true);
        return 0;
    }
    ...
    public int mute(String groupId, UserInfo user, boolean mute) {
        ...
        //发送禁言消息
        IMService.getInstance().sendMessage(groupId, user.userId, IMParams.MSG_TYPE_MUTE, jsonObject.toString(), true, true);
        return 0;
    }
    ...
    public int muteAll(String groupId, boolean mute) {
        ...
        //发送禁言消息
        IMService.getInstance().sendGroupMessage(groupId, IMParams.MSG_TYPE_MUTE_ALL, jsonObject.toString(), true, true);
        return 0;
    }
}
