...
import com.aliyun.auikits.ktv.databinding.KtvActivityLoginBinding;
...
import com.aliyun.auikits.karaoke.room.util.PermissionUtils;
import com.aliyun.auikits.karaoke.room.util.SettingFlags;
import com.aliyun.auikits.karaoke.room.util.SettingFlagsKeyDef;
...
@Route(path = "/karaoke/KaraokeLoginActivity")
public class KaraokeLoginActivity extends AppCompatActivity{
    private static final String TAG = "KtvLoginTag";
    private static final int MAX_ID_LEN = 15;
    private KtvActivityLoginBinding binding;
    ...
    public void onCreate( Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, com.aliyun.auikits.ktv.R.layout.ktv_activity_login);
        ...
        PermissionX.init(this)
                ...
                .request((allGranted, grantedList, deniedList) -> {
                    if(!allGranted) {
                        ToastHelper.showToast(KaraokeLoginActivity.this, "请开通权限后重试", Toast.LENGTH_SHORT);
                        finish();
                    } else {
    ...
    private void login(UserInfo userInfo) {
        ...
        RetrofitManager.getRetrofit(KTVServerConstant.HOST).create(KTVRoomApi.class).login(appSignRequest)
                .flatMap(new Function<LoginResponse, ObservableSource<ImTokenResponse>>() {
                    ...
                    public ObservableSource<ImTokenResponse> apply(LoginResponse response) throws Throwable {
                        if(response.isSuccess()) {
                            KaraokeLoginActivity.this.authorization = "Bearer " + response.token;
                            ImTokenRequest imTokenRequest = new ImTokenRequest(userInfo.userId, userInfo.deviceId, "admin");
                            Log.v(TAG, "LoginSuccess");
                            return RetrofitManager.getRetrofit(KTVServerConstant.HOST).create(KTVRoomApi.class)
    ...
    private void onLoginEnd(boolean success, IMNewToken aliyun_im) {
        ...
        if(success) {
            String userId = SettingFlags.getFlag(SettingFlagsKeyDef.FLAG_PROFILE_ID, "");
            Intent intent = new Intent(this, KaraokeEntryActivity.class);
            intent.putExtra(KaraokeEntryActivity.KEY_AUTHORIZATION, KaraokeLoginActivity.this.authorization);
            intent.putExtra(KaraokeEntryActivity.KEY_IM_TAG, aliyun_im);
            intent.putExtra(KaraokeEntryActivity.KEY_USER_ID, userId);

            startActivity(intent);
        } else {
            ToastHelper.showToast(KaraokeLoginActivity.this, R.string.voicechat_login_failed, Toast.LENGTH_SHORT);
        }
    }
