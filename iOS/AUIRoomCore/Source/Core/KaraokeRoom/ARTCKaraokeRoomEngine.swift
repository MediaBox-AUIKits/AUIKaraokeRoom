...
@objcMembers open class ARTCKaraokeRoomMusicConfig: NSObject {
    ...
    /**
     * 歌曲Id，如果需要播放来自AliMusicContentCenter获取到的歌曲，则设置songID，前提是歌曲必须已经下载好
     */
    ...
    public func loadMusicWithConfig(config: ARTCKaraokeRoomMusicConfig) {
        ...
        if let rtcEngine = self.rtcEngine {
            ...
            if let player = player {
                ...
                player.enablePublish(byDualStream: true)
                player.setProgressInterval(50)
                self.mediaPlayer = player
                self.musicConfig = config
                
                self.updateMusicPrepareStateAndNotify(state: .Preparing)
                if (config.songID != nil && !config.songID!.isEmpty) {
                    player.loadCopyrightedMusicResource(withPosition: config.songID!, startPosition: config.startPosition, delegate: self)
                } else {
        ...
        DispatchQueue.main.async {
            ...
            if let msg = msg {
                if msg.type == .musicProgress {
                    self.onRemotePlayProgressUpdate(remoteProgress: Int64(msg.progress), remoteNtp: msg.networkTime, uid: uid)
                }
                else if msg.type == .custom {
                    self.onRemotePlayStateUpdate(msgData: msg.data, uid: uid)
                }
            }
        }
    }
...
extension ARTCKaraokeRoomEngine {
    ...
    func onLocalPlayProgressUpdate(millisecond: Int64) {
        self.delegate?.onMusicPlayProgressUpdate?(millisecond: millisecond, uid: "", isLocalProgress: true)
     ...
     * 接收到主唱者发送的播放进度消息
     */
    func onRemotePlayProgressUpdate(remoteProgress: Int64, remoteNtp: Int64, uid: String) {
        
        if self.singerRole == .Audience {
            self.delegate?.onMusicPlayProgressUpdate?(millisecond: remoteProgress, uid: uid, isLocalProgress: false)
        }
        else if self.singerRole == .JoinSinger && self.mediaPrepareState == .Prepared {
            let anchorUid = self.delegate?.anchorUidForKaraokeRoomEngine?()
            if anchorUid?.isEmpty ?? true {
                return
            }
            if !uid.isEqual(anchorUid) {
                return
            }
     ...
     * 接收到主唱者发送的播放状态变更消息
     */
    func onRemotePlayStateUpdate(msgData: Data, uid: String) {
