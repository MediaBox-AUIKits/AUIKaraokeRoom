...
@objcMembers open class AUIVoiceRoomViewController: UIViewController {
    ...
    public lazy var bottomView: AUIVoiceRoomBottomView = {
        let view = AUIVoiceRoomBottomView(enableBgMusic: self.enableBgMusic)
    ...
    open var enableBgMusic: Bool {
        return self.roomController.isAnchor
    }
...
extension AUIVoiceRoomViewController: ARTCVoiceRoomEngineDelegate {
    ...
    open func onJoinedMic(seatIndex: Int32, user: ARTCRoomUser) {
        self.insertTips("\(user.getFinalNick()) 上 \(self.micSeatView.getDefaultMicName(seatIndex: seatIndex))")
    ...
    open func onLeavedMic(seatIndex: Int32, user: ARTCRoomUser) {
        self.insertTips("\(user.getFinalNick()) 下 \(self.micSeatView.getDefaultMicName(seatIndex: seatIndex))")
        self.micSeatView.updateMicSeatInfo(seatIndex: seatIndex)
        if user.userId == self.roomController.me.userId {
            self.updateMicState()
