...
open class AUIVoiceRoomMicSeatCell: UIView {
    ...
    open func getDefaultMicName(startIndex: Int32) -> String {
        return "\(self.seatInfo.index + startIndex)\(AUIVoiceRoomBundle.getString("号麦") ?? "号麦")"
    }
    ...
    open func updateSeatUser(startIndex: Int32) {
        if let user = self.seatInfo.user {
            ...
            self.nameLabel.text = self.getDefaultMicName(startIndex: startIndex)
...
open class AUIVoiceRoomMicSeatView: UIView {
    ...
    open func getDefaultMicName(seatIndex: Int32) -> String {
        if let seatCell = self.findMicSeatCell(seatIndex: seatIndex) {
            return seatCell.getDefaultMicName(startIndex: self.isAnchorSeatViewMiddle ? 0 : 1)
        }
        return ""
    }
